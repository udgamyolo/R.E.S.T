...
//declaring the PulseOximeter class
PulseOximeter pox;
...
//declaring structures to store sleep stage begin and end times
struct AwakeRecord {
  uint32_t awakeBegin;
  uint32_t awakeEnd;
};
...
//declaring variables to classify sleep 
...
//variables to mark the sleep stage times
...
//arrays to store the values of each period of each sleep stage
...
//boolean variables to control the flow of the program
bool menu = true;
...
void setup() {
  //starting the M5Core2
  M5.begin();
  Serial.begin(115200);

  //checking for and initializing the Oximeter
...
void loop() {
  ...
  if (menu) {
    ...
    //checking if the user presses the START button
  ...
  // Recording heart rate for 60 seconds to get Average Heart Rate
  ...
  // Average Heart Rate Display Loop
  if ((showing_RestingHeartRate)) {
    ...
    //checking if the user clicks the "Measure Again" button
    ...
    //checking if the user clicks the "Continue" button
...
//prompts the user to start sleeping
  if ((SleepMenu)) {
    touch = M5.Touch.getPressPoint();
    M5.Lcd.setTextColor(TFT_WHITE);
    M5.Lcd.setTextSize(2);
...
//checking if the user is still awake or has drifted into another stage
    if (Awake) {
      //checking if the user presses the stop button
...
//checking if the user is still in Light Sleep or in another stage
    if (LightSleep) {
...
//checking if the user is still in Deep Sleep or in another stage
    if (DeepSleep) {
      if ((CurrentHeartRate <= LightSleepUpper) && (CurrentHeartRate >= LightSleepLower)) {
        DeepSleepEnd = millis();
...
//checking if the user is still in REM Sleep or in another stage
    if (REMSleep) {
      if ((CurrentHeartRate <= LightSleepUpper) && (CurrentHeartRate >= LightSleepLower)) {
        REMSleepEnd = millis();
...
//displays the results of Sleep Tracking
  if (displayResults == true) {
    M5.Lcd.fillTriangle(300, 210, 300, 230, 320, 220, TFT_WHITE);

    touch = M5.Touch.getPressPoint();

//checking if the user wants to go to the next page
